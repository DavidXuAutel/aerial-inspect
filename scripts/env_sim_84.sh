...
export AIRSIM_SCENE_CONFIG="${AIRSIM_SCENE_CONFIG:-configs/sim/bridge_scene.yaml}"
export SIM_DEVICE="${SIM_DEVICE:-cuda}"
export SIM_DETECTOR="${SIM_DETECTOR:-open_vocab}"
# fanout_rgb: native → rgb_yolo (detector) + 224² rgb (WM)
export AERIAL_CAPTURE_W="${AERIAL_CAPTURE_W:-1280}"
export AERIAL_CAPTURE_H="${AERIAL_CAPTURE_H:-720}"
export SIM_YOLO_IMGSZ="${SIM_YOLO_IMGSZ:-1280}"
export SIM_YOLO_MODEL="${SIM_YOLO_MODEL:-yolov8s-worldv2.pt}"
export BRIDGE_CAMERA_PITCH="${BRIDGE_CAMERA_PITCH:--25}"
export SIM_YOLO_CONF="${SIM_YOLO_CONF:-0.1}"
...
echo "[env_sim_84] SCENE=$AIRSIM_SCENE_ID config=$AIRSIM_SCENE_CONFIG"
